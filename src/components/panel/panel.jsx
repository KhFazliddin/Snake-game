...
export default class Panel extends Component {
  ...
  addSnakeLength = () => {
    ...
    switch (direction) {
      case "ArrowUp":
        this.setState({
          snakePositions: [
            { top: lastTop + 10 + "px", left: lastLeft },
            ...snakePositions,
          ],
        });
        break;
      ...
      case "ArrowLeft":
        this.setState({
          snakePositions: [
            { top: lastTop, left: lastLeft + 10 + "px" },
            ...snakePositions,
          ],
        });
        break;
  ...
  checkEat = () => {
    ...
    if (
      parseInt(foods.style.left) === parseInt(headLeft) &&
      parseInt(foods.style.top) === parseInt(headTop)
    ) {
  ...
  checkDie = () => {
    ...
    if (
      parseInt(headTop) < 0 ||
      parseInt(headTop) > 290 ||
      parseInt(headLeft) < 0 ||
      parseInt(headLeft) > 290
    ) {
      alert("Yutqazdiz onam ");
      clearInterval(this.timer);
      return;
    }
    ...
    for (let i = 0; i <= snakePositions.length - 2; i++) {
      if (
        snakePositions[i].top === headTop &&
        snakePositions[i].left === headLeft
      ) {
        alert("Game Over");
        clearInterval(this.timer);
        return;
      ...
      <div className="panel">
        <div className="stage">
          ...
          >
            {snakePositions.map((snakeItem) => {
              return (
                <div
                  key={uuid()}
                  style={{ left: snakeItem.left, top: snakeItem.top }}
                ></div>
              );
            })}
          </div>
          <div
            className="food"
            ref={(c) => (this.foods = c)}
            onClick={this.changFoodPosition}
